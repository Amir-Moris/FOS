...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	if(ret == E_SEMAPHORE_EXISTS) return E_SEMAPHORE_EXISTS;
	struct Semaphore *sem;
	ret = allocate_semaphore_object(&sem);
	if(ret == E_NO_SEMAPHORE) return E_NO_SEMAPHORE;
	sem->empty = 0;
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int ret = get_semaphore_object_ID(ownerEnvID, semaphoreName);
	if(ret == E_SEMAPHORE_NOT_EXISTS) return;
