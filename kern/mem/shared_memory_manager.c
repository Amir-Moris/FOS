...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	for(int i = 0; i < MAX_SHARES; i++) {
		if(shares[i].empty == 0 && shares[i].ownerID == ownerID && strcmp(shareName, shares[i].name) == 0) {
			return E_SHARED_MEM_EXISTS;
		}
	}
	//if(objectID != E_SHARED_MEM_NOT_EXISTS) return E_SHARED_MEM_EXISTS;
	struct Share *allocatedObject;
	// allocate new object
	allocatedObject = virtual_address;
	int objectID = allocate_share_object(&allocatedObject);
	if(objectID == E_NO_SHARE) return E_NO_SHARE;
	// add frame
	uint32 sva = 1 ? (uint32)virtual_address : ROUNDDOWN((uint32)virtual_address, PAGE_SIZE);
	uint32 eva = 1 ? (uint32)virtual_address + size : ROUNDUP((uint32)virtual_address + size, PAGE_SIZE);
	for(int i = sva, indx = 0; i < eva; i+=PAGE_SIZE, indx++) {
		struct FrameInfo *ptr_frame_info;
		allocate_frame(&ptr_frame_info);
		map_frame(myenv->env_page_directory, ptr_frame_info, i, PERM_USER | PERM_WRITEABLE);
		add_frame_to_storage(allocatedObject->framesStorage, ptr_frame_info, indx);
	}
	// initiate allocatedObject
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	if(ret == E_SHARED_MEM_NOT_EXISTS) return E_SHARED_MEM_NOT_EXISTS;
	uint32 sva = 1 ? (uint32)virtual_address : ROUNDDOWN((uint32)virtual_address, PAGE_SIZE);
	uint32 eva = 1 ? (uint32)virtual_address + shares[ret].size : ROUNDUP((uint32)virtual_address + shares[ret].size, PAGE_SIZE);
	for(uint32 i = sva, indx = 0; ; i+=PAGE_SIZE, indx++) {
		struct FrameInfo *ptr_frame_info = get_frame_from_storage(shares[ret].framesStorage, indx);
		if(ptr_frame_info == NULL) break;
		map_frame(myenv->env_page_directory, ptr_frame_info, i, (shares[ret].isWritable * PERM_WRITEABLE) | PERM_USER);
	}
	shares[ret].references++;
	return ret;
...
int freeSharedObject(int32 sharedObjectID, void *startVA)
{
	...
	//	a) 0 if success
	//	b) E_SHARED_MEM_NOT_EXISTS if the shared object is not exists
	int ret = get_share_object_ID(shares[sharedObjectID].ownerID, shares[sharedObjectID].name);
	if (ret == E_SHARED_MEM_NOT_EXISTS) return E_SHARED_MEM_NOT_EXISTS;

	uint32 sva = ROUNDDOWN((uint32)startVA, PAGE_SIZE);
	uint32 eva = ROUNDUP((uint32)startVA + shares[sharedObjectID].size, PAGE_SIZE);
	...
	for(uint32 va = sva; va < eva; va+=PAGE_SIZE) {
		...
		if(ptr_page_table != NULL){
			bool isEmpty = 1;
			for(int entry = 0; entry < NPTENTRIES; entry++) {
				if(ptr_page_table[entry] != 0) {
					isEmpty = 0;
					break;
				}
			}
