...
//
void free_all_tables_from_main_memory(struct Env *e, int inc) {
	for(int i = 0; i < USER_TOP; i+=inc) {
		uint32 *ptr_page_table = NULL;
		int ret = get_page_table(e->env_page_directory, i, &ptr_page_table);
		if (ptr_page_table != NULL) {
			uint32 phys = kheap_physical_address((int)ptr_page_table);
			struct FrameInfo *ptr_frame_info = to_frame_info(phys);
			kfree((void*)ptr_page_table);
			//free_frame(ptr_frame_info);
		}
	}
}
void free_pages_in_page_ws(struct Env *e) {
...
void env_free(struct Env *e)
{
	...
	uint32 sva= 2145382400;
	kfree((void*)sva);
	// [5] Free Shared variables [if any]
	for(int i = 0; i < e->ENV_MAX_SHARES; i++) {
		free_share_object(e->ptr_sharing_variables[i].id_in_shares_array);
	}
	// [6] Free Semaphores [if any]
	for(int semaphoreObjectID = 0; semaphoreObjectID < MAX_SEMAPHORES; semaphoreObjectID++) {
		if(semaphores[semaphoreObjectID].empty) continue;
		if(semaphores[semaphoreObjectID].ownerID == e->env_id) {
			free_semaphore_object(semaphores[semaphoreObjectID].ownerID);
		}
	}
	// [7] Free all TABLES from the main memory
	free_all_tables_from_main_memory(e, PAGE_SIZE * 1024);
